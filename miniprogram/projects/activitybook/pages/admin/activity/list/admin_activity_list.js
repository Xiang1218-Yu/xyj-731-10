...
Page({
	...
	bindBatchDel: function () {
		...
		let callback = async () => {
			try {
				await cloudHelper.callCloudSumbit('admin/activity_batch_del', { ids }, { title: '删除中' });
		...
		pageHelper.showConfirm(`确认批量删除选中的 ${ids.length} 个活动？删除后报名数据将一并删除且不可恢复`, callback);
	},
	...
	bindBatchStatus: function (e) {
		...
		let callback = async () => {
			try {
				await cloudHelper.callCloudSumbit('admin/activity_batch_status', { ids, status }, { title: '处理中' });
		...
		pageHelper.showConfirm(`确认批量${statusDesc}选中的 ${ids.length} 个活动？`, callback);
	},
