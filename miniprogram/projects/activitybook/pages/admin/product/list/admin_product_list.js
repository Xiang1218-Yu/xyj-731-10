...
Page({
	...
	bindBatchDelTap: function () {
		...
		let callback = async () => {
			try {
				await cloudHelper.callCloudSumbit('admin/product_batch_del', { ids: this.data.selectedIds }, { title: '删除中' }).then(res => {
					this._afterBatch('批量删除成功');
				});
			} catch (e) { console.log(e); }
		}
		pageHelper.showConfirm('确认删除选中的 ' + this.data.selectedIds.length + ' 项？删除不可恢复', callback);
	},
	...
	bindBatchStatusTap: function (e) {
		...
		let callback = async () => {
			try {
				await cloudHelper.callCloudSumbit('admin/product_batch_status', { ids: this.data.selectedIds, status }, { title: '处理中' }).then(res => {
					this._afterBatch('操作成功');
				});
			} catch (e) { console.log(e); }
		}
		pageHelper.showConfirm('确认对选中的 ' + this.data.selectedIds.length + ' 项执行此操作？', callback);
	},
